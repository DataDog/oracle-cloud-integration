...
output "namespace_error" {
  ...
  value       = length(local.derived_namespaces) > 0 ? "" : "Could not obtain any namespaces and compartments to send the metrics. Make sure that the provided compartments contains the resources for the supported metrics namespaces"
}
